...
def city_report(df_city_sel,df_fact_sel):
    ...
    4) Do not report a city in the final report if no prescirber is assigned to it
    ...
    try:
        logger.info(f"Transformation of data is started...\n\n")
        df_city_split=df_city_sel.withColumn('zip_counts',column_split_cnt(df_city_sel.zips))
        df_fact_grp = df_fact_sel.groupBy(df_fact_sel.presc_state, df_fact_sel.presc_city).agg(countDistinct("presc_id").alias("presc_counts"),
...
def top_5_Prescribers(df_fact_sel):
    ...
    """
    try:
        logger.info("Transformation for top_5 Prescribers() is started...\n\n")
